...
// G9: two atds created using the class from the atd.h header file
ATD *atd1 = new ATD();

ATD *atd2 = new ATD();
void partition();                                 // G9: function definition, finds the optimal partition out of all possibilities, and halves the old utility values at every 250,000 cycle
void atd_insert(PACKET *packet, int cpu, int set) // G9: Function inserts blocks in the ATD and maintains the replacement policy for the ATDs
...
int atdupdate(PACKET *packet, int set, int cpu) // G9: Updates the hit counters of the ATD if there is a hit and inserts the upcoming block if there is a miss
// G9: return the lru value of the hit block
{
...
void atdfill(PACKET *packet, int cpu, uint32_t set) // G9: matches the tag for checking hit or miss and calls the partitioning function for
// optimised partitioning
{
  ...
  if (cpu == 0)
  {
    ...
    if (current_core_cycle[0] % 250000 == 0)
      partition(); // calling the partition algorithm, which updates the partition
    return;
  }
  else
  {
    ...
    if (current_core_cycle[1] % 250000 == 0)
      partition(); // calling the partition algorithm, which updates the partition
    return;
  }
}
...
void partition() // G9:Function finds the optimal values of ways to be allocated to each core
{
  ...
  if (current_core_cycle[0] % 2500000 == 0)
  {
    ...
    // cout << "Current Partition:" << ucp1 << ", " << ucp2 << "\n";
  }
}
...

{
  ...
  if (set % 64 == 0)
    ...
    atdfill(packet, packet->cpu, set / 64); // G9: Function atdfill called on every 2^6 th set out of 2 ^ 12
  // G9: Called here as check_hit invoked everytime a block enters LLC from upper level cache
